# Spec — agent-backpressure

## Purpose

Establish the layered backpressure that catches AI-agent and human regressions before they reach `main`: typed cross-stack contracts (OpenAPI client, basedpyright, vue-tsc), unit and Playwright smoke coverage in CI, a single on-demand `verify` entry point, Conventional Commits enforcement, an MCP-driven UI verification path for interactive agents, and a README quality-checks table that documents where each mechanism runs.
