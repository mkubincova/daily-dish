...
async def test_create_tag_requires_auth(client: AsyncClient):
    resp = await client.post("/tags", json={"name": "spicy"})
    assert resp.status_code == 401


@pytest.mark.asyncio
async def test_create_tag_normalises_name(auth_client: AsyncClient):
    resp = await auth_client.post("/tags", json={"name": "  Spicy   Dish "})
...
async def test_create_tag_lowercase(auth_client: AsyncClient):
    resp = await auth_client.post("/tags", json={"name": "HEARTY"})
    assert resp.status_code == 201
...
async def test_create_tag_idempotent(auth_client: AsyncClient):
    r1 = await auth_client.post("/tags", json={"name": "christmas"})
    assert r1.status_code == 201
    r2 = await auth_client.post("/tags", json={"name": "Christmas"})
...
async def test_create_tag_empty_name_rejected(auth_client: AsyncClient):
    resp = await auth_client.post("/tags", json={"name": "   "})
    assert resp.status_code == 422


@pytest.mark.asyncio
async def test_list_tags_ordered_alphabetically(auth_client: AsyncClient):
    await auth_client.post("/tags", json={"name": "zucchini"})
    await auth_client.post("/tags", json={"name": "apple"})
    await auth_client.post("/tags", json={"name": "mango"})
